...
export const TopBar = ({ readOnly }) => {
	...
	const confirmEditSillabus = async (store, actions) => {
		...
		// Validaciones específicas de los días
		...
		// Si hay errores, mostramos el primero y salimos
		if (errors.length > 0) {
			await swal({ ...errors[0], icon: "error", button: "OK" });
			actions.setSyllabusErrors(invalidDays); // Guardamos los errores específicos si existen
			return false;
		}

		// Si no hay errores, limpiamos los errores y continuamos
		...
		if (willEdit) {
			try {
				// Guardamos el syllabus (false significa guardar bajo la misma versión)
	...
	const confirmSaveSillabus = async (store, actions) => {
		// Validar si el slug está vacío o no hay días
		if (store.info.slug === "") {
			swal({
				title: "Syllabus details can't be empty",
				text: "Please fill the syllabus details to save",
		...
		// Validación de campos de los días
		const invalidDays = store.days.filter(day => {
			const hasInvalidFields =
